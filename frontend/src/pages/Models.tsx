...
import { Download, ExternalLink, FolderSearch, HardDrive, Info, LoaderCircle, MoreHorizontal, Play, RefreshCw, Search, ShieldCheck, Trash2, XCircle } from "lucide-react";
...
export default function Models({ engine, models, refresh, toast }: { engine: EngineStatus; models: Model[]; refresh: () => void; toast: (s: string, b?: boolean) => void }) {
  ...
  useEffect(() => {
    if (managed) api<any>("/api/models/catalog").then((value) => setCatalog(value.items));
  }, [managed]);
  useEffect(() => {
  ...
  const discover = async () => {
    setSearching(true);
    try {
      const value = await api<any>(`/api/models/search?query=${encodeURIComponent(query)}`);
  ...
  const download = async (repo: string) => {
    try {
      const value = await post<any>("/api/models/download", { repoId: repo });
      setJob(value); toast("Download started");
    } catch (error: any) { toast(error.message, true); }
  };
  ...
  return <div className="page models-page">
    <div className="page-heading"><div><div className="eyebrow">CHECKPOINTS / LOCAL STORAGE</div><h1>Models</h1><p>Discover known-good checkpoints, then download and launch them in place.</p></div><button className="secondary" onClick={async () => { await post("/api/models/rescan"); refresh(); toast("Library rescanned"); }}><RefreshCw size={16} />Rescan</button></div>
