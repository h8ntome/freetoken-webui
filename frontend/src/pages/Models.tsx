...
export default function Models({ engine, models, refresh, toast }: { engine: EngineStatus; models: Model[]; refresh: () => void; toast: (s: string, b?: boolean) => void }) {
  ...
  return <div className="page models-page">
    ...
    {history.length > 1 && <label>Download history <select value={job?.id || ""} onChange={e => api<any>(`/api/jobs/${e.target.value}`).then(setJob).catch(e => toast(e.message, true))}>{history.map(item => <option key={item.id} value={item.id}>{item.payload.repoId} · {item.id.slice(0, 8)}</option>)}</select></label>}
